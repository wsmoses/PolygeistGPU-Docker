...
for OMP in `echo ${POW6} ${FAC6_48} | sed -e 's/ /\n/g' | sort -n`; do
	for BS in `echo ${POW8} ${FAC12_288} | sed -e 's/ /\n/g' | sort -n`; do
		...
		timeout --kill-after=30s 10m bash -c "${PRELOAD} python3 -m benchmarker --framework=pytorch --problem=conv2d --mode=inference --problem_size=$((4*${BS})),3,512,512 --batch_size=${BS} ${RUNVERS}" > ${TMPLOG} 2>&1
